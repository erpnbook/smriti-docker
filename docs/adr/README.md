---
Document ID: "ADR-INDEX"
Title: "SMRITI Architecture Decision Records (ADR) Registry"
Owner: "Development Team"
Audience: "Architect"
Module: "Core"
Version: "1.0.0"
Status: "Active"
Primary Document: "Yes"
Depends On: ""
Related Modules: ""
Last Updated: "2026-07-04"
Last Reviewed: "2026-07-04"
AI Generated: "No"
Reviewed By: "Jawahar R. Mallah"
---

# SMRITI Architecture Decision Records (ADR) Registry

This registry tracks the status and expiration criteria of all active and historical SMRITI Architecture Decision Records.

| ADR ID | Title | Status | Violates | Expiry | Status Checked |
|---|---|---|---|---|---|
| [ADR-0002](./ADR-0002.md) | SMRITI Business Layer Independence | ACTIVE | None | Permanent | 2026-07-04 |
| [ADR-0008](./ADR-0008.md) | Barcode Studio Modularization | ACTIVE | None | Permanent | 2026-07-04 |

---

## Author Profile

- **Author**: Jawahar R. Mallah
- **Designation**: Founder & Chief Architect
- **Organization**: AITDL – AI Technology & Development Lab
- **Professional Experience**: 20+ Years of Experience in Software Development, Retail Technology, Distribution Systems, POS Solutions, ERP Implementations, Business Process Automation, and Enterprise Application Design.

> "Always decision-ready."  
> — Jawahar R. Mallah  
> Founder & Chief Architect, AITDL
